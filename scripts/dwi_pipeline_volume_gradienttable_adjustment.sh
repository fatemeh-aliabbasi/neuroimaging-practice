...
dwiextract dwi_den_unr.mif -bzero - | mrconvert - -coord 3 0 ap_b0.mif -force #taking out the ap_b0.mif
mrconvert b0_pa_unr.mif -coord 3 0 pa_b0.mif -force  #taking out the first b0 volume out of 5 volumes
mrconvert b0_pa_unr.mif -coord 3 4 vol5_pa_b0.mif -force #taking out the 5th volume for visually check(this image should be dark)
echo "0 0 0 0" > pa_grad.b #make the gradient table: one row per volume, format is x y z b 
mrconvert pa_b0.mif pa_b0_grad.mif -grad pa_grad.b #write it into the header of the pa_b0.mif
mrcat ap_b0.mif pa_b0_grad.mif -axis 3 b0s_paired.mif -force
